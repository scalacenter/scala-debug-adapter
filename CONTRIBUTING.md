# Contributing to Scala Debug Adapter

The Scala Scala Debug Adapter project welcomes contributions from anybody wishing to participate. All code or documentation that is provided must be licensed with the same license that Scala Debug Adapter is licensed with (Apache 2.0, see [LICENSE](./LICENSE.md)).

## Code of Conduct

People are expected to follow the [Scala Code of Conduct](./CODE_OF_CONDUCT.md) when discussing Scala Debug Adapter on the Github page, Gitter channel, or other venues.
...
## Previewing Microsite

Follow installation instructions for `jekyll` at https://47degrees.github.io/sbt-microsites/docs/getting-started/

    sbt "; unidoc ;site/makeMicrosite ;site/run"

## Publishing Microsite

    sbt -DGITHUB_TOKEN=[token] ci-doc-release
...
```
Copyright (c) $today.year the Scala Debug Adapter contributors.
