...
abstract class StepFilterTests(protected val scalaVersion: ScalaVersion) extends DebugTestSuite {
  ...
  test("should not step into synthetic method of case class") {
    ...
    check(
      ...
      StepOut.line(10),
      Breakpoint(11),
      StepIn.method("A.<init>(String): void"),
      StepOut.line(11),
      Breakpoint(12),
      StepIn.method(
  ...
  test("should step into default value method") {
    ...
    check(
      ...
      StepOut.line(18),
      StepOut.line(8),
      StepIn.method("B.<init>(String): void")
    )
  }
  ...
  test("should match on vararg type") {
    ...
    check(
      Breakpoint(8),
      StepIn.method("A.<init>(Seq): void"),
      Breakpoint(9),
      StepIn.method(if (isScala3) "Predef.println(x: Any): Unit" else "Predef$.println(Object): void"),
      Breakpoint(10),
      StepIn.method("StringContext.<init>(Seq): void"),
      Breakpoint(11),
      StepIn.method(if (isScala3) "Predef.println(x: Any): Unit" else "Predef$.println(Object): void")
    )
  }
  ...
  test("step out of class loader and into constructor") {
    val source =
      """|package example
         ...
         |""".stripMargin
    implicit val debuggee: TestingDebuggee = TestingDebuggee.mainClass(source, "example.Main", scalaVersion)
    check(Breakpoint(7), StepIn.method("A.<init>(): void"))
  }
