...
private[internal] object RuntimeEvaluatorBooleanPatterns {
  ...
  object Module {
    ...
    def unapply(field: Field): Option[ClassType] = {
      field.`type`() match {
    ...
    def unapply(tree: RuntimeValidationTree): Option[ClassType] =
      tree match {
  ...
  object MethodCall {
    def unapply(tree: Valid[RuntimeValidationTree]): Option[RuntimeValidationTree] =
      tree.toOption.filter(_.hasMethodCall)
  }

  object ReferenceTree {
    def unapply(tree: Validation[RuntimeValidationTree]): Option[ReferenceType] = {
