...
class Scala3UnpicklerStats extends munit.FunSuite:
  ...
  test("scala3-compiler:3.3.0"):
    ...
    for
      ...
      // if cls.name == "dotty.tools.repl.package$$anon$1"
      clsSym <- cls match
    ...
    // if method.name == "dotty$tools$dotc$reporting$UniqueMessagePositions$$super$markReported"
    do
      method match
    ...
    checkCounter(methodCounter, 56848, expectedAmbiguous = 34, expectedNotFound = 1009)
