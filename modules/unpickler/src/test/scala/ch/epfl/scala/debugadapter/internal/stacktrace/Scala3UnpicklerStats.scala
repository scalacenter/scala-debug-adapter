...
class Scala3UnpicklerStats extends munit.FunSuite:
  ...
  test("scala3-compiler:3.3.0"):
    ...
    for
      ...
      // if cls.name == "dotty.tools.repl.ScriptEngine$Factory"
      clsSym <- cls match
    ...
    // if method.name == "getMethodCallSyntax"
    do
      method match
    ...
    checkCounter(methodCounter, 56850, expectedAmbiguous = 34, expectedNotFound = 1007)
