...
object Dependencies {
  ...
  val scala212 = scalaEnvVersion.filter(isScala212).getOrElse("2.12.21")
  val scala213 = scalaEnvVersion.filter(isScala213).getOrElse("2.13.18")
  val scala30 = scalaEnvVersion.filter(isScala30).getOrElse("3.0.2")
  val scala31Plus = scalaEnvVersion.filter(isScala31Plus).getOrElse("3.3.7")
  ...
  val sbtTestAgent = "org.scala-sbt" % "test-agent" % "1.12.10"
  val scalaMeta = ("org.scalameta" %% "parsers" % "4.16.1").cross(CrossVersion.for3Use2_13)
